...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        sum_sol = np.copy(f.weights)
        best_sol, best_sol_output = np.copy(f.weights), f.compute_output(X=X, y=y)
        delta = self.tol_
        for iter in range(1, self.max_iter_ + 1):
            if delta < self.tol_:
                break
            ...
            weights -= eta * grad
            f.weights = weights
            delta = np.linalg.norm(weights - previous_weights, 2)
            sum_sol += np.copy(weights)
            last_iter = iter

            if val < best_sol_output:
                best_sol, best_sol_output = np.copy(f.weights), np.copy(val)

            self.callback_(solver=self, weights=np.copy(weights), val=np.copy(val),
                           grad=np.copy(grad), t=iter, eta=eta, delta=delta)
